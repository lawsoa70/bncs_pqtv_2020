...
int tracebar::revertiveCallback( revertiveNotify * r )
{
	...
	int sourceIndex = sltRevertive.getNamedParam("index").toInt();

	debug("tracebar::revertiveCallback device: %1 source: %2 destination: %3", r->device(), sourceIndex, r->index());
	...
	if( (r->device() == m_iterationDevice) && ( r->index() == m_iterationDestination ))
	{
		...
		if( sourceIndex > 0 )
		{
			routerName( m_iterationDevice, 0, sourceIndex, m_iterationRouterName);
			debug("tracebar::revertiveCallback() start iteration with router name=%1", m_iterationRouterName);
			timerStart(1, 1);
		}
		else
		{
	...
	if( m_dynamicUpdates )
	{
		...
		if( it != m_reverseMap.end() )
		{
			...
			if( tally != sourceIndex )
			{
				debug( "tracebar::+++ got new tally for position %1 of %2 (was %3)", pos, sourceIndex, tally);

				// remember the new tally in the map
				it->second = MAKELONG(pos, sourceIndex);
				...
				if( !initialSetup )
				{
					...
					routerName(m_iterationDevice, m_iterationDatabase, sourceIndex, m_iterationRouterName);
...
{
	debug("tracebar::iteration %1 %2 %3", m_iterationDevice, m_iterationRouterName, m_iterationDatabase);
	int iIndexPos;
	...
	if( m_iterationDatabase == DB_DESTINATION )
	{
		...
		debug("tracebar::iteration() POLL dest %1", m_iterationDestination);
		m_iterationDatabase = DB_SOURCE;
	...
	else if (m_iterationDatabase == DB_SOURCE)
	{
		...
		//debug("tracebar::iteration() LOOKUP connection");
...
bool tracebar::showName( void )
{
	...
	//debug("tracebar::Appeding adjustnames: %1 %2 %3 %4", m_iterationDevice, m_iterationDatabase ? DB_DESTINATION_ADJUST : DB_SOURCE_ADJUST, iSlot, sAdjustName);
